...
function AreaAccordion({ area, index }) {
  ...
  return (
    <div
      className={`cg-area card area-folder card-area ${open ? "cg-area--open" : ""}`}
    >
      <button
      ...
      >
        <div className="pe-3 overflow-hidden flex-grow-1 text-start">
          {area.pre_name && (
            <span className="cg-area__pre-name d-block mb-1">
              {area.pre_name}
            </span>
          )}
        ...
        <div className="cg-area__header-right d-flex align-items-center gap-3">
          {hasChildren && !open && (
            <span className="cg-area__badge d-none d-md-inline-block">
              ...
              {area.children.length === 1 ? "dependencia" : "dependencias"}
            </span>
          )}
...
export default function ContactGuide({ areas }) {
  ...
  if (!areas || areas.length === 0) {
    return (
      <div className="cg-empty" key="empty-initial">
        <span>
  ...
  return (
    <div className="cg">
      ...
      {filtered.length === 0 ? (
        <div className="cg-empty" key="empty-results">
          <span>
        ...
        <div className="cg-list" key="results-list">
