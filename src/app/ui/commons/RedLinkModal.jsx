...
const MEDIOS_DE_PAGO_URL = "https://economiariocuarto.gob.ar/medios-de-pago";

export default function RedLinkModal({ section }) {
  const [isOpen, setIsOpen] = useState(false);
  const storageKey = `redlink_interrupcion_popup_shown_${section}`;

  useEffect(() => {
    // El padre (Server Component) ya decide si corresponde montar este modal
    // según la ventana de fechas; esta verificación es solo un resguardo.
    if (!isRedLinkActive()) return;
  ...
  const handleMoreInfo = () => {
    handleClose();
    window.open(MEDIOS_DE_PAGO_URL, "_blank", "noopener,noreferrer");
  };
